...
export class FormSupplierComponent implements OnInit {
  ...
  constructor(private service: SupplierService,  private responsePage: ResponsePage, public searchOptions: SearchOptions, public supplier: Supplier) {
    ...
    this.service.findBySearchPaginable(this.searchOptions).subscribe(
      data => {
        this.responsePage = data,
          this.listOfSuppliers = this.responsePage.content,
  ...
  public edit(): void {
    ...
    this.supplier = this.listOfSuppliers[this.selectedRow];
    this.toogle = 'form';
  ...
  public delete(): void {
    ...
    this.service.delete(this.listOfSuppliers[this.selectedRow].id).subscribe(
      result => console.log('Deleted'),
      error => alert(error),
      () => {
        this.closeDialog();
        this.listOfSuppliers.splice(this.selectedRow, 1);
      }
    );
  }
...
public save():void{
   if (this.isNewRecord)
    {
        ...
        data => {
          ...
          this.listOfSuppliers.push(this.supplier),
          this.closeDialog();
        },
        error => {
          switch(error.status){
            case 422:
            {
              alert('Ya existe una suppliero con el código ingresado');
              break;
            }
            default:
            {
  ...
  /*private sortResults(prop, asc) {
    this.listOfSuppliers = this.listOfSuppliers.sort(function(a, b) {
      if (asc) {
